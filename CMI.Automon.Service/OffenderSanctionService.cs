...
namespace CMI.Automon.Service
{
    public class OffenderSanctionService : IOffenderSanctionService
    {
        ...
        public int SaveOffenderSanctionDetails(string CmiDbConnString, OffenderSanction offenderSanctionDetails)
        {
            if (automonConfig.IsDevMode)
            {
                ...
                return offenderSanctionDetails.Id == 0 ? new Random().Next(0, 10000) : offenderSanctionDetails.Id;
            }
            else
            {
                using (SqlConnection conn = new SqlConnection(CmiDbConnString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        cmd.Parameters.Add(new SqlParameter()
                        {
                            ...
                            Value = offenderSanctionDetails.Id,
                            IsNullable = true
                        });
                        cmd.Parameters.Add(new SqlParameter()
                        {
                        ...
                        cmd.Parameters.Add(new SqlParameter()
                        {
                            ...
                            Value = offenderSanctionDetails.EventDateTime,
                            IsNullable = false
                        });
                        cmd.Parameters.Add(new SqlParameter()
                        {
