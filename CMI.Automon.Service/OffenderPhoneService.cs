...
namespace CMI.Automon.Service
{
    public class OffenderPhoneService : IOffenderPhoneService
    {
        ...
        public IEnumerable<OffenderPhone> GetAllOffenderPhones(string CmiDbConnString, DateTime? lastExecutionDateTime, DataTable officerLogonsToFilterTbl)
        {
        ...
        public int SaveOffenderPhoneDetails(string CmiDbConnString, OffenderPhone offenderPhoneDetails)
        {
            if (automonConfig.IsDevMode)
            {
                ...
                return offenderPhoneDetails.Id == 0 ? new Random().Next(0, 10000) : offenderPhoneDetails.Id;
            }
            else
            {
                using (SqlConnection conn = new SqlConnection(CmiDbConnString))
