...
Description:	To get all offender phone details from given source database
---------------------------------------------------------------------------------
Test execution:-
EXEC	
	...
	BEGIN
		...
		FROM
			[$SourceDatabaseName].[dbo].[Person] P JOIN [$SourceDatabaseName].[dbo].[Offender] O
				ON P.[Id] = O.[PersonId]
				JOIN [$SourceDatabaseName].[dbo].[PersonPhone] PP
					ON P.[Id] = PP.[PersonId]
					JOIN [$SourceDatabaseName].[dbo].[PhoneNumber] PN
