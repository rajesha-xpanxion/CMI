...
{
    public class OutboundClientProfileGPSSupervisionProcessor : OutboundBaseProcessor
    {
        private readonly IOffenderGPSViolationService offenderGPSViolationService;
        private readonly ICommonService commonService;

        public OutboundClientProfileGPSSupervisionProcessor(
        ...
        public override TaskExecutionStatus Execute(IEnumerable<OutboundMessageDetails> messages, DateTime messagesReceivedOn)
        {
            Logger.LogInfo(new LogRequest
            {
                ...
                MethodName = "Execute",
                Message = "GPS Supervision activity processing initiated."
            });

            TaskExecutionStatus taskExecutionStatus = new TaskExecutionStatus
            {
                ProcessorType = Common.Notification.ProcessorType.Outbound,
                TaskName = "GPS Supervision",
                IsSuccessful = true,
            ...
            try
            {
                if (messages.Any())
                {
                    Logger.LogDebug(new LogRequest
                    {
                        OperationName = this.GetType().Name,
                        MethodName = "Execute",
                        Message = string.Format("{0} GPS Supervision messages received for processing.", messages.Count()),
                ...
                foreach (OutboundMessageDetails message in messages)
                {
                    ...
                    try
                    {
                        ...
                        offenderGPSViolationDetails = (OffenderGPSViolation)ConvertResponseToObject<ClientProfileGPSSupervisionDetailsActivityResponse>(
                            message.ClientIntegrationId,
                            message.ActivityIdentifier,
                            message.AutomonIdentifier,
                            RetrieveActivityDetails<ClientProfileGPSSupervisionDetailsActivityResponse>(message.Details),
                        ...
                        else
                        {
                            Logger.LogDebug(new LogRequest
                            {
                                OperationName = this.GetType().Name,
                                MethodName = "Execute",
                                Message = "Compliant GPS Supervision message received.",
                                AutomonData = JsonConvert.SerializeObject(offenderGPSViolationDetails),
                                NexusData = JsonConvert.SerializeObject(message)
                            });
                            message.IsSuccessful = true;
                    ...
                    catch (CmiException ce)
                    {
                        ...
                        Logger.LogWarning(new LogRequest
                        {
                            OperationName = this.GetType().Name,
                            MethodName = "Execute",
                            Message = "Error occurred while processing a Client Profile - GPS Supervision Details activity.",
                            Exception = ce,
                            AutomonData = JsonConvert.SerializeObject(offenderGPSViolationDetails),
                            NexusData = JsonConvert.SerializeObject(message)
                        });
                    ...
                    catch (Exception ex)
                    {
                        ...
                        Logger.LogError(new LogRequest
                        {
                            OperationName = this.GetType().Name,
                            MethodName = "Execute",
                            Message = "Critical error occurred while processing a Client Profile - GPS Supervision Details activity.",
                            Exception = ex,
                            AutomonData = JsonConvert.SerializeObject(offenderGPSViolationDetails),
                            NexusData = JsonConvert.SerializeObject(message)
                        });
            ...
            catch (Exception ex)
            {
                ...
                Logger.LogError(new LogRequest
                {
                    OperationName = this.GetType().Name,
                    MethodName = "Execute",
                    Message = "Critical error occurred while processing Client Profile - GPS Supervision Details activities.",
                    Exception = ex,
            ...
            Logger.LogInfo(new LogRequest
            {
                OperationName = this.GetType().Name,
                MethodName = "Execute",
                Message = "GPS Supervision activity processing completed.",
