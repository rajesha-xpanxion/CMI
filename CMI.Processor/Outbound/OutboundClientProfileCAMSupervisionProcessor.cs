...
{
    public class OutboundClientProfileCAMSupervisionProcessor : OutboundBaseProcessor
    {
        private readonly IOffenderCAMViolationService offenderCAMViolationService;
        private readonly ICommonService commonService;

        public OutboundClientProfileCAMSupervisionProcessor(
        ...
        public override TaskExecutionStatus Execute(IEnumerable<OutboundMessageDetails> messages, DateTime messagesReceivedOn)
        {
            Logger.LogInfo(new LogRequest
            {
                ...
                MethodName = "Execute",
                Message = "CAM Supervision activity processing initiated."
            });

            TaskExecutionStatus taskExecutionStatus = new TaskExecutionStatus
            {
                ProcessorType = Common.Notification.ProcessorType.Outbound,
                TaskName = "CAM Supervision",
                IsSuccessful = true,
            ...
            try
            {
                if (messages.Any())
                {
                    Logger.LogDebug(new LogRequest
                    {
                        OperationName = this.GetType().Name,
                        MethodName = "Execute",
                        Message = string.Format("{0} CAM Supervision messages received for processing.", messages.Count()),
                ...
                foreach (OutboundMessageDetails message in messages)
                {
                    ...
                    try
                    {
                        ...
                        offenderCAMViolationDetails = (OffenderCAMViolation)ConvertResponseToObject<ClientProfileCAMSupervisionDetailsActivityResponse>(
                            message.ClientIntegrationId,
                            message.ActivityIdentifier,
                            message.AutomonIdentifier,
                            RetrieveActivityDetails<ClientProfileCAMSupervisionDetailsActivityResponse>(message.Details),
                        ...
                        else
                        {
                            Logger.LogDebug(new LogRequest
                            {
                                OperationName = this.GetType().Name,
                                MethodName = "Execute",
                                Message = "Compliant CAM Supervision message received.",
                                AutomonData = JsonConvert.SerializeObject(offenderCAMViolationDetails),
                                NexusData = JsonConvert.SerializeObject(message)
                            });
                            message.IsSuccessful = true;
                    ...
                    catch (CmiException ce)
                    {
                        ...
                        Logger.LogWarning(new LogRequest
                        {
                            OperationName = this.GetType().Name,
                            MethodName = "Execute",
                            Message = "Error occurred while processing a Client Profile - CAM Supervision Details activity.",
                            Exception = ce,
                            AutomonData = JsonConvert.SerializeObject(offenderCAMViolationDetails),
                            NexusData = JsonConvert.SerializeObject(message)
                        });
                    ...
                    catch (Exception ex)
                    {
                        ...
                        Logger.LogError(new LogRequest
                        {
                            OperationName = this.GetType().Name,
                            MethodName = "Execute",
                            Message = "Critical error occurred while processing a Client Profile - CAM Supervision Details activity.",
                            Exception = ex,
                            AutomonData = JsonConvert.SerializeObject(offenderCAMViolationDetails),
                            NexusData = JsonConvert.SerializeObject(message)
                        });
            ...
            catch (Exception ex)
            {
                ...
                Logger.LogError(new LogRequest
                {
                    OperationName = this.GetType().Name,
                    MethodName = "Execute",
                    Message = "Critical error occurred while processing Client Profile - CAM Supervision Details activities.",
                    Exception = ex,
            ...
            Logger.LogInfo(new LogRequest
            {
                OperationName = this.GetType().Name,
                MethodName = "Execute",
                Message = "CAM Supervision activity processing completed.",
